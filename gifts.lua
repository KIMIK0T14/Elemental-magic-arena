-- KIMIKO BETA - Módulo de Regalos
-- Funciones: Auto Gift, Notificaciones, Lista de Regalos
...
local function teleportToGift(part) 
    ...
    Data.hrp.CFrame = part.CFrame + Vector3.new(0, 3, 0) 
end
...
local function showGiftNotification(num, part)
    ...
    local function create()
        local n = Instance.new("TextButton", notificationContainer) 
        n.Size = UDim2.new(1, 0, 0, 70) 
        n.Position = UDim2.new(1, 50, 0, #activeNotifications * 75) 
        n.BackgroundColor3 = Colors.Gift 
        n.BorderSizePixel = 0 
        n.Text = "" 
        n.ZIndex = 101 
        ...
        ic.Position = UDim2.fromOffset(10, 15) 
        ic.Text = "🎁" 
        ic.TextSize = 28 
        ...
        tl.Size = UDim2.new(1, -70, 0, 25) 
        tl.Position = UDim2.fromOffset(55, 10) 
        tl.Text = Texts.giftAppeared .. num 
        ...
        local sl = Instance.new("TextLabel", n) 
        sl.Size = UDim2.new(1, -70, 0, 20) 
        sl.Position = UDim2.fromOffset(55, 38) 
        sl.Text = Texts.tapToGo 
        sl.TextColor3 = Color3.fromRGB(230, 230, 230) 
...
function Module.init(contentFrame)
    ...
    local gui = Data.gui
    local isSpanish = Data.isSpanish
    
    -- Toggle Auto Gift
    local autoGiftToggle = createToggle(contentFrame, Texts.autoGift, UDim2.fromOffset(10, 15), autoGift, Colors.Gift)
    
    autoGiftToggle.switch.MouseButton1Click:Connect(function() 
        autoGift = not autoGift 
        Data.EnabledFeatures["AutoGift"] = autoGift 
        if autoGift then 
    ...
    local notificationToggle = createToggle(contentFrame, Texts.giftNotifications, UDim2.fromOffset(10, 65), giftNotifications, Colors.Gift)
    
    notificationToggle.switch.MouseButton1Click:Connect(function() 
        giftNotifications = not giftNotifications 
        Data.EnabledFeatures["GiftNotifications"] = giftNotifications 
    ...
    -- Título de lista
    local giftListTitle = Instance.new("TextLabel", contentFrame) 
    giftListTitle.Size = UDim2.new(1, -20, 0, 25) 
    giftListTitle.Position = UDim2.fromOffset(10, 115) 
    giftListTitle.Text = Texts.availableGifts 
    giftListTitle.TextColor3 = Colors.Text 
    giftListTitle.BackgroundTransparency = 1 
    giftListTitle.Font = Enum.Font.GothamBold 
    giftListTitle.TextSize = 13 
    giftListTitle.TextXAlignment = Enum.TextXAlignment.Left
    
    -- Lista de regalos
    local giftListFrame = Instance.new("ScrollingFrame", contentFrame) 
    giftListFrame.Size = UDim2.new(1, -20, 0, 280) 
    giftListFrame.Position = UDim2.fromOffset(10, 145) 
    giftListFrame.BackgroundColor3 = Colors.Surface 
    giftListFrame.BorderSizePixel = 0 
    giftListFrame.ScrollBarThickness = 6 
    giftListFrame.ScrollBarImageColor3 = Colors.Gift 
    Instance.new("UICorner", giftListFrame).CornerRadius = UDim.new(0, 12)
    
    local giftListLayout = Instance.new("UIListLayout", giftListFrame) 
    giftListLayout.Padding = UDim.new(0, 6) 
    giftListLayout.SortOrder = Enum.SortOrder.LayoutOrder
    
    local giftListPadding = Instance.new("UIPadding", giftListFrame) 
    giftListPadding.PaddingTop = UDim.new(0, 6) 
    giftListPadding.PaddingBottom = UDim.new(0, 6) 
    giftListPadding.PaddingLeft = UDim.new(0, 6) 
    giftListPadding.PaddingRight = UDim.new(0, 6)
    
    -- Funciones de lista
    local function clearGiftList() 
        for _, b in pairs(giftButtons) do b:Destroy() end 
        table.clear(giftButtons) 
    end
    ...
    local function addGiftToList(g, n) 
        ...
        nm.Size = UDim2.new(1, -45, 0, 20) 
        nm.Position = UDim2.fromOffset(38, 5) 
        nm.Text = (isSpanish and "Regalo " or "Gift ") .. n 
        ...
        local tp = Instance.new("TextLabel", btn) 
        tp.Size = UDim2.new(1, -45, 0, 15) 
        tp.Position = UDim2.fromOffset(38, 24) 
        tp.Text = Texts.tapToGo 
    ...
    print("[KIMIKO] Módulo Gifts inicializado")
end
